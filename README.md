...
Beim Scrollen (Desktop) bzw. Antippen (Mobile) passiert Folgendes gleichzeitig:

1. Ein schwarzes Panel (`.detail-panel`) fährt vom unteren Bildschirmrand hoch — gesteuert über eine reine `height`-Transition in CSS.
2. Das Vorschaubild (`.media`) wird währenddessen vertikal gestaucht (`transform: scaleY(...)`, Ankerpunkt oben), sodass es sichtbar "zusammengedrückt" wirkt und Platz für das Panel macht.

Beide Werte sind fest in CSS hinterlegt und nicht per JavaScript synchronisiert — sie wurden so gewählt, dass sie optisch zusammenpassen:

```css
.project.revealed .media { transform: scaleY(0.6); }      /* Desktop */
.project.revealed .detail-panel { height: clamp(300px, 46vh, 480px); }

/* Mobile, in der @media (max-width: 768px)-Regel: */
.project.revealed .media { transform: scaleY(0.46); }
.project.revealed .detail-panel { height: clamp(340px, 60vh, 620px); }
```

Möchtest du den Effekt stärker oder dezenter haben, an diesen vier Werten drehen (kleinerer `scaleY`-Wert = stärkere Stauchung, größere `height` = mehr Platz für Text).
...
- **Desktop:** Pfeiltasten `←`/`→` bzw. `↑`/`↓`, sowie ein Klick auf das Foto (nur solange das Panel noch geschlossen ist — ist es offen, übernehmen die Panel-Buttons oder die Pfeiltasten die Navigation).

...
- Klickt man von dort auf **„Work“**, springt man zurück zum ersten Projekt (Lettering for friedaschreibt), um die Arbeiten erneut durchzugehen.
...
└── images/
    ├── Kreuzbild-1.jpg     ← Vorschaubild "Lettering for friedaschreibt" (Hintergrund)
    ├── Kreuzbild-2.jpg     ← Karussel-Bild "Lettering for friedaschreibt"
    ├── Kreuzbild-3.jpg     ← Karussel-Bild "Lettering for friedaschreibt"
    ├── Kreuzbild-4.jpg     ← Karussel-Bild "Lettering for friedaschreibt"
...
- `.info-head`: `<h2>` (Projekttitel) und `<span class="tag">` (Kategorie/Jahr, z. B. `Type Design, 2025`) — beide stehen in **einer Zeile**, z. B. „Lettering for friedaschreibt, Type Design, 2025". Das Komma zwischen Titel und Tag wird automatisch per CSS eingefügt (`.info-head .tag::before`), im HTML also nur `<h2>Titel</h2><span class="tag">Kategorie, Jahr</span>` eintragen, ohne Komma davor. Direkt danach folgt optional das Karussel.
- `.info-body`: `<p>` (Beschreibung) — steht bewusst weiter rechts als die Titelspalte (siehe Abschnitt „Layout des Detail-Panels" weiter unten)

Die Platzhaltertexte (Lorem Ipsum) sind aktuell bewusst länger als am Anfang (~5x), damit sich das Zusammenstauchen des Bildes und das interne Scrollen im Panel realistisch testen lassen. Beim Eintragen der echten Texte einfach den `<p>`-Inhalt ersetzen — kürzer oder länger ist beides problemlos möglich, das Panel scrollt bei Bedarf automatisch. Es können auch mehrere `<p>`-Absätze hintereinander in `.info-body` stehen (siehe "Lettering for friedaschreibt" als Beispiel) — der Abstand zwischen den Absätzen wird automatisch per CSS ergänzt (`.info-body p + p`).
...
Aktuell hat nur das erste Projekt ("Lettering for friedaschreibt") ein Karussel — als Muster für weitere Projekte (dort mit drei Bildern: `Kreuzbild-2.jpg` bis `Kreuzbild-4.jpg`). Es sitzt bewusst direkt unter dem Projekttitel in `.info-head` (nicht mehr in `.info-body`), weil dort sonst viel leerer Raum entsteht — die Titelspalte ist schmaler als die Textspalte und war ohne Karussel nur halb gefüllt. Um es bei einem anderen Projekt zu ergänzen, folgenden Block innerhalb von `.info-head`, direkt nach dem `<h2>`, einfügen:
...
- Die Größe wird zentral über `.carousel` in der CSS gesteuert (`max-width: 420px`, Seitenverhältnis 3:2) und gilt automatisch für jedes Projekt, das einen Karussel-Block bekommt.
- Projekte ohne `.carousel`-Block funktionieren weiterhin genau wie bisher, ganz ohne Karussel.

## Layout des Detail-Panels: Abstand, Spalten und Schrift

Ein paar bewusste Layout-Entscheidungen im `.info`-Bereich, falls sie später nochmal angepasst werden sollen:

- **Sicherheitsabstand zur fixen Navigation:** Das Panel (`.detail-panel`) kann strukturell nie ganz bis an den oberen Bildschirmrand wachsen — seine Höhe ist über `max-height: calc(100% - var(--nav-safe))` gedeckelt (`--nav-safe`, aktuell `76px`, im `:root`-Block). Dieselbe Reserve gilt für den kontinuierlichen Scroll-Effekt auf Desktop über die JS-Konstante `NAV_SAFE` (ebenfalls `76`, ganz am Anfang des `<script>`-Blocks) — **beide Werte müssen synchron gehalten werden**, falls sich die Nav-Position (`.top-nav`, `top`-Wert) mal ändert. Dadurch bleibt ganz oben immer Platz für die fixe Nav ("Loris Klinke" / "Work") reserviert, egal wie weit das Panel geöffnet ist — und das Padding von `.detail-panel-inner` (`clamp(8px, 1.4vh, 14px)`) kann dafür ganz eng bleiben, sodass der Text wirklich direkt am Bild klebt (wie im veryes.co-Screenshot als Vorlage).
- **Tag, Titel und Fließtext in einer gemeinsamen, sehr kleinen Größe:** Über die CSS-Variable `--info-text-size` (aktuell `12.5px`, mobil `12px`) haben Kategorie/Jahr-Tag, Projekttitel und Beschreibungstext dieselbe, bewusst kleine Schriftgröße — orientiert an der sehr kompakten Bildunterschrift im veryes.co-Screenshot statt eines großen Headline-Titels. Titel und Tag stehen dabei in einer Zeile (z. B. „Lettering for friedaschreibt, Type Design, 2025") und bleiben trotzdem optisch unterscheidbar: der Titel in Fettung (`font-weight: 600`) und Weiß, der Tag danach kursiv und gedämpfter eingefärbt. Beide sind linksbündig direkt unter dem Vorschaubild platziert.
- **Zwei-Spalten-Grid mit deutlichem Versatz:** `.info` ist ein Grid mit fester Titelspalte (`minmax(260px, 420px)`, bietet auch dem Karussel genug Platz) und einer flexiblen Textspalte. Die Textspalte (`.info-body`) bekommt zusätzlich `margin-left` (bis zu 160px, abhängig von der Fensterbreite), damit die Beschreibung deutlich weiter rechts beginnt statt direkt neben dem Titel — angelehnt an das Layout von veryes.co. Der Textblock selbst ist etwas großzügiger geschnitten (`max-width: 58ch`, Zeilenabstand `1.8`) für ein weitläufigeres Lesegefühl. Auf Mobile wird der Versatz automatisch auf `0` zurückgesetzt (einspaltiges Layout).
- **Editoriale Schrift durchgängig:** Tag, Titel und Beschreibung nutzen alle die CSS-Variable `--editorial-font` (aktuell `'Newsreader', Georgia, serif`, per Google Fonts eingebunden) statt der Mono-Schrift, die weiterhin nur für die Navigation verwendet wird. `--editorial-font` ist aktuell ein Platzhalter, der optisch in Richtung der Schrift auf leoneichelbaum.de/biography-&-bibliography/ geht — der exakte Font von dort ließ sich nicht automatisiert auslesen (keine Zugriffsmöglichkeit auf das gerenderte CSS der fremden Seite, gleiche Einschränkung gilt für veryes.co). Bei Bedarf einfach in der `:root`-Definition durch den echten Font-Namen ersetzen (siehe Abschnitt „Farben und Schrift" weiter unten) und ggf. den passenden Google-Fonts-Link im `<head>` austauschen.
...
Die gesamte Sektion nutzt jetzt dieselbe `--editorial-font` wie die Projekttexte im „Work"-Bereich (über `font-family` auf `.cv-inner` gesetzt, vererbt sich automatisch an alle Kind-Elemente wie Name, Fließtext, Stationen etc.).
...
Skills sind einzelne `<li>`-Einträge in `.cv-tags` (werden als schlichter, mit Kommas getrennter Fließtext dargestellt — keine Boxen oder Rahmen mehr; das Komma nach jedem Eintrag kommt automatisch per CSS), Sprachen folgen dem gleichen einfachen Muster wie im bestehenden Code (`.cv-list-plain`).
...
Kleinerer Wert = noch schnelleres Durchswitchen; der zugehörige `setTimeout` in `slideTo()` (aktuell `300`ms) sollte dabei ungefähr zur CSS-Dauer passen, damit die Aufräum-Logik nicht zu früh greift.
...
:root {
  ...
  --accent: #6b6bff;     /* Akzentfarbe (nur noch Fokus-Rahmen im Karussell) */
  --display-font: 'Roboto Mono', monospace;   /* aktuell ungenutzt, aus Kompatibilitaet stehen gelassen */
  --body-font: 'Roboto Mono', monospace;      /* Basis-Schrift im Rest der Seite */
  --editorial-font: 'Newsreader', Georgia, serif; /* Tag, Titel + Beschreibungstext im Detail-Panel, Platzhalter (s. o.) */
  --info-text-size: 12.5px; /* gemeinsame Groesse fuer Tag, Titel und Beschreibungstext (mobil auf 12px reduziert) */
  --nav-safe: 76px; /* oben reservierter Platz fuer die fixe Nav - muss zu NAV_SAFE im JS (Anfang des <script>-Blocks) passen */
}
```
...
- **„Work“** springt zurück zum ersten Werk (Lettering for friedaschreibt) — Standard-Ansicht beim Laden der Seite.
- Der aktive Reiter wird durch ein gefülltes weißes Rechteck mit schwarzer Schrift hervorgehoben (`.nav-item.active`), der inaktive Reiter bleibt schlicht mit gedämpft-weißer Schrift ohne Hintergrund. Das entspricht immer dem Abschnitt, in dem man sich gerade befindet — nicht der Aktion des Buttons.

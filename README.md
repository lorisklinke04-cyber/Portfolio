...
- **Sicherheitsabstand zur fixen Navigation:** `.detail-panel-inner` hat oben ein Padding von `clamp(76px, 10vh, 100px)`. Das ist kein zufälliger Wert, sondern bewusst groß genug, damit Tag/Titel niemals mit der fixen Nav oben links ("Loris Klinke" / "Work") überlappen — auch dann nicht, wenn das Panel komplett geöffnet ist und sein Inhalt bis ganz an den oberen Bildschirmrand reicht (an dem Punkt liegt der Panel-Inhalt exakt am Viewport-Anfang, im selben Bereich wie die Nav). Falls die Nav-Position (`.top-nav`, `top`-Wert) später geändert wird, sollte dieser Wert entsprechend mit angepasst werden.
- **Tag, Titel und Fließtext in einer gemeinsamen Größe:** Über die CSS-Variable `--info-text-size` (aktuell `16.5px`, mobil `15px`) haben Kategorie/Jahr-Tag, Projekttitel und Beschreibungstext dieselbe Schriftgröße — bewusst kein großer Headline-Titel mehr, sondern ein durchgängig ruhiges, redaktionelles Schriftbild. Tag und Titel bleiben trotzdem optisch als "Label" erkennbar: der Tag steht kursiv, der Titel in Fettung (`font-weight: 600`). Beide sind linksbündig direkt unter dem Vorschaubild platziert.
...
:root {
  ...
  --info-text-size: 16.5px; /* gemeinsame Groesse fuer Tag, Titel und Beschreibungstext (mobil auf 15px reduziert) */
}
```

...
- **Enger Abstand zum Bild:** `.detail-panel-inner` hat oben nur noch wenig Padding (`clamp(10px, 1.6vh, 18px)`), damit Titel und Text wie eine Bildunterschrift direkt unter dem Vorschaubild sitzen, statt mit großem Leerraum darüber.
- **Zwei-Spalten-Grid mit Versatz:** `.info` ist ein Grid mit fester Titelspalte (`minmax(260px, 380px)`) und einer flexiblen Textspalte. Die Textspalte (`.info-body`) bekommt zusätzlich `margin-left` (bis zu 90px, abhängig von der Fensterbreite), damit die Beschreibung sichtbar weiter rechts beginnt statt direkt neben dem Titel — angelehnt an das Layout von veryes.co. Auf Mobile wird dieser Versatz automatisch auf `0` zurückgesetzt (einspaltiges Layout).
- **Editoriale Schrift für Titel und Fließtext:** Titel (`.info-head h2`) und Beschreibung (`.info-body p`) nutzen die CSS-Variable `--editorial-font` (aktuell `'Newsreader', Georgia, serif`, per Google Fonts eingebunden) statt der Mono-Schrift, die weiterhin für Navigation und Tag/Kategorie-Label verwendet wird. `--editorial-font` ist aktuell ein Platzhalter, der optisch in Richtung der Schrift auf leoneichelbaum.de/biography-&-bibliography/ geht — der exakte Font von dort ließ sich nicht automatisiert auslesen (keine Zugriffsmöglichkeit auf das gerenderte CSS der fremden Seite). Bei Bedarf einfach in der `:root`-Definition durch den echten Font-Namen ersetzen (siehe Abschnitt „Farben und Schrift" weiter unten) und ggf. den passenden Google-Fonts-Link im `<head>` austauschen.
...
:root {
  ...
  --display-font: 'Roboto Mono', monospace;   /* Navigation, Tag/Kategorie-Label */
  --body-font: 'Roboto Mono', monospace;      /* Basis-Schrift im Rest der Seite */
  --editorial-font: 'Newsreader', Georgia, serif; /* Titel + Beschreibungstext im Detail-Panel, Platzhalter (s. o.) */
}
```

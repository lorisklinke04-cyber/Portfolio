...
1. **Werke** (`<section class="project work">`) — die vier Projekte Meridian, Fracture, Artifact, Assembly. Jedes zeigt zunächst nur das Vorschaubild. Beim Scrollen (Desktop) bzw. Antippen (Mobile) fährt von unten ein schwarzes **Detail-Panel** hoch, das die Projektinfos zeigt — währenddessen wird das Bild darüber sichtbar zusammengestaucht (angelehnt an veryes.co). Innerhalb des Panels gibt es „‹ Vorheriges Projekt“ / „Nächstes Projekt ›“, um direkt zum Nachbarprojekt zu springen, ohne das Panel erst schließen zu müssen.
...
- `.info-body`: `<p>` (Beschreibung), optional das Karussel, und darunter die „Vorheriges/Nächstes Projekt“-Buttons
...
Einen kompletten `<section class="project work">`-Block kopieren oder löschen. **Wichtig:**

- die Klasse `work` muss erhalten bleiben, sonst taucht das Projekt nicht im Klick-/Wisch-Zyklus der Werke auf. Der Lebenslauf-Abschnitt bleibt davon unberührt, da er keine `work`-Klasse trägt.
- jedes Projekt braucht seine eigenen `.panel-prev`/`.panel-next`-Buttons am Ende von `.info` (einfach aus einem bestehenden Projekt mitkopieren) — die Sprung-Logik dahinter läuft automatisch über JavaScript, es muss nichts angepasst werden.
...
Auf **Desktop** ist der Effekt jetzt direkt an die Scrollbewegung gekoppelt, nicht mehr nur ein einmaliges An/Aus:
...
const SQUEEZE_MIN = 0.6;  // Bild-Skalierung (scaleY) bei voll geöffnetem Panel — kleiner = stärkere Stauchung
```

Beide Konstanten stehen direkt am Anfang des `<script>`-Blocks in `index.html` und lassen sich dort anpassen.

**Auf Mobile** bleibt es beim einfachen Antippen: ein Tap öffnet/schließt das Panel komplett (kein kontinuierliches Scrollen, da Touch-Scrollen für das Wischen zwischen Projekten gebraucht wird) — dafür sorgt weiterhin die CSS-Klasse `.revealed` mit den entsprechenden `@media (max-width: 768px)`-Werten.

### Zwischen Projekten navigieren, ohne das Panel zu schließen

Innerhalb jedes offenen Panels gibt es „‹ Vorheriges Projekt“ und „Nächstes Projekt ›“ (`.panel-prev` / `.panel-next`). Damit kommt man auch **rückwärts** durch die Werke, nicht nur vorwärts. Zusätzlich funktionieren weiterhin:

- **Desktop, Vollansicht (Bild unverzerrt, Panel geschlossen):** Ein Klick auf die **obere Bildschirmhälfte** springt zum vorherigen Projekt, ein Klick auf die **untere Bildschirmhälfte** zum nächsten. Das funktioniert nur, solange `revealProgress` bei 0 liegt — sobald man scrollt und sich das Bild verzerrt, übernehmen stattdessen die Panel-Buttons oder die Pfeiltasten.
- **Desktop, jederzeit:** Pfeiltasten `←`/`→` bzw. `↑`/`↓` springen zum vorherigen/nächsten Projekt, auch bei offenem Panel.
- **Mobile:** Wischen nach oben/unten zwischen den Projekten funktioniert jederzeit, unabhängig davon, ob das Panel gerade offen ist.

### Übergangs-Animation zwischen zwei Projekten (Desktop)

Der Wechsel zwischen zwei Werken ist ein **smoothes Wischen** statt einer reinen Überblendung: Das neue Bild schiebt sich rein, während das alte in dieselbe Richtung rausgeschoben wird (`next` = von unten nach oben, `prev` = von oben nach unten). Das steuert die Funktion `slideTo()` im JavaScript, indem sie kurzzeitig beide Sektionen gleichzeitig sichtbar macht und ihnen ein `transform: translateY(...)` gibt, das per CSS-Transition (`0.75s`) auf `0` bzw. ganz aus dem Bild animiert.

Ausgelöst wird das Wischen von: den Panel-Buttons, den Pfeiltasten und den Klickzonen oben/unten. Der Sprung zurück zum ersten Projekt über den „Work“-Reiter bleibt bewusst eine einfache Überblendung ohne Richtung, da er kein Nachbar-Projekt ist, sondern ein Rücksprung zum Anfang.

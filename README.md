...
- `.info-head`: `<h2>` (Projekttitel) und `<span class="tag">` (Kategorie/Jahr, z. B. `Photography, 2024`) — beide stehen in **einer Zeile**, z. B. „Meridian, Photography, 2024". Das Komma zwischen Titel und Tag wird automatisch per CSS eingefügt (`.info-head .tag::before`), im HTML also nur `<h2>Meridian</h2><span class="tag">Photography, 2024</span>` eintragen, ohne Komma davor. Direkt danach folgt optional das Karussel.
...
Skills sind einzelne `<li>`-Einträge in `.cv-tags` (werden als schlichte, eckige Rahmen-Rechtecke dargestellt — bewusst kein abgerundetes Pill-Design), Sprachen folgen dem gleichen einfachen Muster wie im bestehenden Code (`.cv-list-plain`).
